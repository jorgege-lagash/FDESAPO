...
export type CategoryState = BaseResourceState<number>;
const initialState: CategoryState = intitializeBaseResource();

export default (state = initialState, action: Action): CategoryState => {
  switch (action.type) {
    ...
    default:
      return cleanLocalStateAfterLogout<CategoryState>(state, action, {
        ...initialState,
      });
  }
};
