...
export type TagState = BaseResourceState<number>;
const initialState: TagState = intitializeBaseResource();

export default (state = initialState, action: Action): TagState => {
  switch (action.type) {
    ...
    default:
      return cleanLocalStateAfterLogout<TagState>(state, action, {
        ...initialState,
      });
  }
};
