import { intitializeBaseResource } from 'src/utils';
import { types } from '../actions/deal.action';
import dealReducer, { DealState } from './deal.reducer';

test('fetchDeals', () => {
  const mockState: DealState = intitializeBaseResource();
  let nextState = dealReducer(mockState, {
    type: types.FETCH_LIST_SUCCESS,
    payload: {
      ids: [3, 4, 5],
    },
  });
  expect(nextState.list).toEqual([3, 4, 5]);
  nextState = dealReducer(mockState, {
    type: types.FETCH_LIST_SUCCESS,
    payload: {
      ids: undefined,
    },
  });
  expect(nextState.list).toEqual([]);
});

test('fetch deal', () => {
  const mockState: DealState = {
    ...intitializeBaseResource(),
    list: [2, 6],
  };
  const nextState = dealReducer(mockState, {
    type: types.FETCH_SUCCESS,
    payload: {
      ids: [30],
    },
  });
  expect(nextState.list).toEqual([2, 6, 30]);
});

test('fetch deals failure', () => {
  const error = {
    statusCode: 404,
    message: 'ups!',
  };
  const mockState: DealState = {
    ...intitializeBaseResource(),
  };
  const nextState = dealReducer(mockState, {
    type: types.FETCH_LIST_FAILURE,
    payload: {
      error,
    },
  });
  expect(nextState.error).toEqual(error);
});
