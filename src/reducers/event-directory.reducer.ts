...
export type EventDirectoryState = BaseResourceState<number>;
const initialState: EventDirectoryState = intitializeBaseResource();

export default (state = initialState, action: Action): EventDirectoryState => {
  switch (action.type) {
    ...
    default:
      return cleanLocalStateAfterLogout<EventDirectoryState>(state, action, {
        ...initialState,
      });
  }
};
