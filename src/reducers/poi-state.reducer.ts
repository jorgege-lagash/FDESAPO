...
import { types } from '../actions/poi-state.action';
...
export type PoiStateState = BaseResourceState<number>;
const initialState: PoiStateState = intitializeBaseResource();

export default (state = initialState, action: Action): PoiStateState => {
  switch (action.type) {
    ...
    default:
      return cleanLocalStateAfterLogout<PoiStateState>(state, action, {
        ...initialState,
      });
  }
};
